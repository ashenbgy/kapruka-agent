...
export function DeliveryForm({
  ...
  const [localCity, setLocalCity] = useState(city || "Kandy");
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!localCity || !localDate) {
      setError("Enter a delivery city and date.");
      return;
    }
    ...
    try {
      setDeliveryDetails(localCity, localDate);
  ...
  return (
    <section>
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        <div>
          <label htmlFor="city" className="text-sm text-zinc-300">
            Delivery city
          </label>
        ...
        <button
          type="submit"
          disabled={loading || !localCity || !localDate}
